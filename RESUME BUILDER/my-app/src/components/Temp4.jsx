...
const Temp4 = () => {
    ...
    projects: [
      {
        ...
        dates: "2022",
        description:
          "Led a brand revitalization campaign that resulted in a significant increase in brand recognition and customer loyalty.",
      },
    ],
  });
  ...
  const createResume = async () => {
    ...
    try {
      setLoading(true);
      const response = await axios.post(`${API_BASE_URL}/api/temp4/create`, {
        resumeData: resume,
      });
  ...
  const saveResume = async () => {
    ...
    try {
      setLoading(true);
      const response = await axios.post(`${API_BASE_URL}/api/temp4/save`, {
        resumeData: resume,
      });
      if (response.data?.data?._id) {
        setResume(response.data.data);
  ...
  const enhanceSingleField = async (field) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}/api/temp4/enhanceField`,
        {
          resumeId: resume._id,
          field,
        }
            ...
            >
              {visibleFields.name && (
                <div
                  ...
                  onBlur={(e) =>
                    setResume({ ...resume, name: e.target.textContent })
                  }
                >
                  {resume.name}
                </div>
              ...
              {visibleFields.designation && (
                <div
                  ...
                  suppressContentEditableWarning
                  onBlur={(e) =>
                    setResume({ ...resume, designation: e.target.textContent })
                  }
                >
                  {resume.designation}
                </div>
                ...
                >
                  ...
                  <div
                    ...
                    onBlur={(e) =>
                      setResume({
                        ...resume,
                        summary: e.target.textContent.split("\n"),
                      })
                    }
                  >
                    {resume.summary.join("\n")}
