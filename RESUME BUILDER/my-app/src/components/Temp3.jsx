...
const Temp3 = () => {
  ...
  const [resumeData, setResumeData] = useState({
    ...
    location: "Location",
    summary:
      "Briefly explain why you are a great fit for this role. I am a strong fit for the web developer role because of my comprehensive understanding of both frontend and backend technologies, including expertise in JavaScript, React, Node.js, and modern web development frameworks.",
    experience: [
      {
        title: "Your Title",
        ...
        description:
          "Company Description. Web development company specializing in creating innovative, user-centric digital solutions.",
        accomplishment:
          "Your accomplishment. Contributed significantly to high-profile projects.",
      },
    ],
    education: [
      {
    ...
    achievements: [
      {
        ...
        describe:
          "Describe what you did. Consistently demonstrated exceptional technical skills and leadership.",
      },
    ],
    courses: [
      {
    ...
    projects: [
      {
        ...
        duration: "Date period",
        description:
          "Summary of your work. A weather app built with React to display real-time weather information.",
      },
    ],
  });
    ...
    const loadSavedResume = async () => {
      ...
      try {
        const response = await axios.get(
          "http://localhost:5000/api/resume1/load",
          {
  ...
  const handleUserContent = (section, key, value, index = null) => {
    ...
    setResumeData((prevData) => {
      ...
      if (section === "skills") {
        updatedData.skills = sanitizedValue
          .split(",")
  ...
  const createResume = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:5000/api/resume1/create",
        { resumeData }
      );
  ...
  const saveResume = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:5000/api/resume1/save",
        { resumeData }
      );
      if (response.data?.data?._id) {
        setResumeData((prev) => ({ ...prev, _id: response.data.data._id }));
  ...
  const enhanceSingleField = async (field) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:5000/api/resume1/enhanceField",
        {
  ...
  return (
    <div className="flex gap-4 text-xs">
      <div className="hidden md:block w-1/4 bg-gray-800 text-white p-4 min-h-screen">
        {loading && (
      ...
      <div className="editResume w-6/7 bg-white p-4">
        <div id="resumeBody">
